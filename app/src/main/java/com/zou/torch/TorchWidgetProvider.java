...
public class TorchWidgetProvider extends AppWidgetProvider
{
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        if (intent.hasCategory(Intent.CATEGORY_ALTERNATIVE))
        {
            ...
            if (mButtonId == 0)
            {
                ...
                mPendingIntent.putExtra("sos", mPreferences.getBoolean("widget_sos" + mWidgetId, false));
        ...
        else if (intent.getAction().equals(TorchSwitch.TORCH_STATE_CHANGED))  this.updateAppWidget(context);
    }

    public void updateAppWidget(Context context)
    {
        AppWidgetManager mWidgetManager = AppWidgetManager.getInstance(context);
        int[] appWidgetIds = mWidgetManager.getAppWidgetIds(new ComponentName(context, getClass()));
        for (int appWidgetId : appWidgetIds) this.updateState(context, appWidgetId);
    }
    ...
    public void updateState(Context context, int appWidgetId)
    {
        ...
        Intent mStateIntent = context.registerReceiver(null, new IntentFilter(TorchSwitch.TORCH_STATE_CHANGED));
