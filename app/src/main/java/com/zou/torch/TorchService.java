...
public class TorchService extends Service
{
    ...
    public void onCreate()
    {
        ...
        mTorchTask = new TimerTask() {
            public void run() {
                ...
                else
                {
                    while(true)
                    {
                        if(!MainActivity.mTorchOn)
                        {
                            FlashDevice.getInstance(mContext).setFlashMode(FlashDevice.OFF);
                            break;
                        }
                        if (MainActivity.light > 500) FlashDevice.getInstance(mContext).setFlashMode(FlashDevice.ON);
                        else FlashDevice.getInstance(mContext).setFlashMode(FlashDevice.OFF);
                        try
                        {
                            Thread.sleep(5);
                        }
        ...
        mStrobeRunnable = new Runnable()
        {
            ...
            public void run()
            {
                ...
                {
                    if (mCounter-- < 1) FlashDevice.getInstance(mContext).setFlashMode(mFlashMode);
                }
                else
                {
        ...
        mSosOffRunnable = new Runnable()
        {
            public void run()
            {
                ...
                mSosCount++;
                if (mSosCount == 9) mSosCount = 0;
                if (mSosTimer != null) mSosTimer.schedule(mSosTask, mSosCount == 0 ? 2000 : 200);
            }
        };
    ...
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        ...
        if (myStrobePeriod == 0) myStrobePeriod = 1;
        mStrobePeriod = (666 / myStrobePeriod) / 4;

        if (mSos) mStrobe = false;
        Log.d(TAG, "onStartCommand mStrobe = " + mStrobe + " mStrobePeriod = " + mStrobePeriod + " mSos = " + mSos);

        if (mSos) mSosTimer.schedule(mSosTask, 0);
        else if (mStrobe) mStrobeTimer.schedule(mStrobeTask, 0, mStrobePeriod);
        else mTorchTimer.schedule(mTorchTask, 0, 100);
